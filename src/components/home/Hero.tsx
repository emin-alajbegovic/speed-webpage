...
const CountUp = ({ end, suffix }: { end: number; suffix: string }) => {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      el.textContent = (end >= 1000 ? Math.floor(start / 1000) + 'k' : Math.floor(start).toString()) + suffix;
...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-screen flex items-center pt-[var(--navbar-height)] overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="max-w-4xl">
          ...
          <motion.p
            ...
            animate="visible"
            custom={2}
            className="text-lg sm:text-xl text-[var(--muted-foreground)] max-w-2xl mb-8 leading-relaxed"
          >
            {t.hero.subtitle}
          </motion.p>
          ...
          >
            {trustBadges.map(({ icon: Icon, label }) => (
              <div
                key={label}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-[var(--card)] border border-[var(--border)] rounded-lg text-xs font-semibold text-[var(--muted-foreground)]"
              >
          ...
          >
            {[
              { value: 20, suffix: '+', label: t.hero.stat1 },
              { value: 50000, suffix: '+', label: t.hero.stat2 },
